...
class LaneDetector:
    ...
    def __init__(self, config):
        ...
        # Lightweight telemetry for downstream state machine
        self.last_left_found = False
        self.last_right_found = False
        self.last_left_points = 0
        self.last_right_points = 0
    ...
    def _transform_image(self, frame):
        ...
        """
        # Resize to 820x410 (exactly matching original calibration resolution)
        resize_frame = cv2.resize(frame, (820, 410))
        ...
        # --- Grayscale Reflection Suppression ---
        gray = cv2.cvtColor(resize_frame, cv2.COLOR_BGR2GRAY)
        # Suppress bright reflections (white spots) which appear very bright
        _, reflection_mask = cv2.threshold(gray, 220, 255, cv2.THRESH_BINARY)
        # Remove these reflections from our detected yellow lane
        binary_image = cv2.bitwise_and(binary_image, cv2.bitwise_not(reflection_mask))
        # ----------------------------------------
    ...
    def _draw_lines(self, img, original_img):
        ...
        # Save point counts before regression clears points downstream
        self.last_left_points = len(self.left_points)
        self.last_right_points = len(self.right_points)

        find_line_right = self._regression_right()
        find_line_left = self._regression_left()

        self.last_left_found = bool(find_line_left)
        self.last_right_found = bool(find_line_right)
    ...
    # ──────────────────────────────────────────────────────────────
    # UTILITY: Side Camera Yellow Detection
    # ──────────────────────────────────────────────────────────────

    def detect_yellow_lane(self, frame):
        """
        Special detection for side cameras (CSI Left/Right).
        Returns (is_found: bool, offset: float)
        offset: -1.0 (left) to 1.0 (right) relative to frame center.
        """
        if frame is None:
            return False, 0.0

        # Preprocess side frame: resize to 820x410
        resize_side = cv2.resize(frame, (820, 410))
        hsv = cv2.cvtColor(resize_side, cv2.COLOR_BGR2HSV)
        
        # Use same adaptive threshold logic or just config baseline
        lower, upper = self._adaptive_threshold(resize_side)
        mask = cv2.inRange(hsv, lower, upper)
        
        # Morphological cleanup
        mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, self.morph_kernel)
        
        # Find horizontal center of mass in bottom half
        h, w = mask.shape
        roi = mask[h//2:h, :]
        moments = cv2.moments(roi)
        
        if moments["m00"] > self.cfg.cv.min_pixels * 1.5: # Threshold reduced for side view
