...
def main_loop():
    ...
    try:
        # We'll use a counter to switch between clear and obstacle states.
        tick_counter = 0
        ...
        while True:
            ...
            # To integrate with ROS2 and QCar2 hardware, you would replace the mock functions
            # with actual data acquisition calls. For example:
            ...
            # ── MOCK DATA FOR TESTING AVOIDANCE SCENARIO ──────────────────
            # Simulate a clear path for the first few seconds, then introduce an obstacle.
            if tick_counter < 100:
                # Mock a clear path initially (no obstacle detected)
                lidar_angles, lidar_dists = _lidar_clear()
                depth_mm                  = _depth_clean()
            else:
                # Mock a real obstacle after 100 ticks (approx. 3-4 seconds)
                lidar_angles, lidar_dists = _lidar_obstacle_centre(d=0.9) # obstacle at 0.9m
                depth_mm                  = _depth_obstacle(m=0.8)       # obstacle at 0.8m

            imu_yaw                   = 0.0                   # mock yaw for simplicity
            encoder_ticks             = int(time.time()*100)  # mock encoder ticks
...
if __name__ == "__main__":
    if "--run" in sys.argv:
        # To run the live control loop, use: python3 script_name.py --run
        # Remember to replace mock data with real sensor inputs first.
        main_loop()
    else:
        # To run unit tests on mock data, use: python3 script_name.py
        ok = run_tests()
