...
# ---------------------------------------------------------------------------
# PURE PURSUIT NAVIGATION CONFIGURATION
# ---------------------------------------------------------------------------
@dataclass
class PurePursuitConfig:
    """
    Geometry and control limits for Pure Pursuit trajectory tracking.
    """
    wheelbase: float = 0.256                # distance between front and rear axles (m)
    lookahead_distance: float = 0.8         # how far ahead on the path to aim (m)
    base_velocity: float = 0.1              # base straight-line speed (m/s)
    min_velocity: float = 0.05              # speed allowed in sharp turns (m/s)
    curve_slowdown_factor: float = 0.5      # determines how aggressively to brake in turns
    steering_alpha: float = 0.4             # EMA smoothing factor for steering (0=full smooth, 1=no smooth)
    max_steering_rate: float = 0.3          # max steering change per step (rad) to prevent oscillations
