...
# How often to save map images (every N iterations)
SAVE_INTERVAL = 50

if __name__ == '__main__':
    # Object initialization (only when run as main script)
    ...
    try:
        # Main loop - iterates until B is pressed on the gamepad
        while gpad.B != 1:
            new = gpad.read()
            start = time.time()

            # RT controls speed of the car, Left stick controls turning left or right
            ...
            # Save map image periodically (not every iteration)
